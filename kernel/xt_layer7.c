...
static int app_data_offset(const struct sk_buff *skb)
{
	...
	if (iphdr->protocol == IPPROTO_TCP) {
		tcphdr = skb_header_pointer(skb, offset, sizeof(*tcphdr), &tcphdr_tmp);
		if (!tcphdr)
			return -1;
		...
		return offset + 8;/* ICMP header is 8 bytes */
	}
	else {
		if (net_ratelimit())
...
static int __init xt_layer7_init(void)
{
	...
	proc_create_data("layer7_numpackets", 0644, init_net.proc_net, &layer7_numpackets_proc_fops, NULL);
	if(maxdatalen < 1) {
	...
	return xt_register_matches(xt_layer7_match, ARRAY_SIZE(xt_layer7_match));
}
